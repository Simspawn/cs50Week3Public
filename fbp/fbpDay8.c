...
/*
    1/28/2025
    Eventualy I need to use less pointers. Kinda defeats the purpose of encapsulation if everything is a pointer. I think.
    We now take up to three word arguments. I think. It compiles. I'll test it real quick...
    Oh, right, it doesn't yet do anything with the third string.
    ...
    We may have to continue this portion tomorrow ((again)) as per below.

    <sigh> I guess that means a state system after all~ Just breathe, and start small. Start
    with a simple current state to hold just that. Need a get_State and get_Stage function as well...

    **!** Updated state idea **!**
    Perhaps state can just be a struct that can contain the other structs it needs. Maybe I can
    even pass entire structs into an instance of State called currentState which is just a struct that
    can hold all relevant fields and struct information.
*/
